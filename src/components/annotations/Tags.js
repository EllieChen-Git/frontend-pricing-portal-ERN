...
class Tags extends Component {
  ...
  render() {
    let elements = this.state.tags.map(tag => (
      <Tag key={tag._id} handleSelect={this.props.handleSelect} tag={tag} />
    ));
    return (
      <div>
        Tags: {elements} |<button onClick={this.reloadTags}>Reload</button>
        <button onClick={() => this.props.handleSelect(null)}>
          Clear Selected
        </button>{" "}
        |
        ...
        {this.props.selectedTag && (
          <h3>Selected tag: {this.props.selectedTag.title}</h3>
        )}
      </div>
    );
  }
